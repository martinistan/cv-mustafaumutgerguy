...
function setup(){
    ...
    function sendCode(){
        ...
        pExp=Date.now()+115*60*1000;
        clearTimeout(aTimer);
        aTimer=setTimeout(function(){if(pCode){logHook('⏰ Kod Geldi Ama Giriş Yapılmadı','Doğrulama kodu gönderildi ancak 115 dk içinde giriş yapılmadı.',10066329,[]);pCode=null;}},115*60*1000);
        cvHook('🔐 Doğrulama Kodu Gönderildi','CV Paneline girmek için önce yarramı yalamalı sonra kodu girmelisin. 🍆\nŞifre doğrulandı, Discord\'tan kod bekleniyor.',5793266,[{name:'🔢 KOD',value:'# **'+pCode+'**\n*(115 dakika geçerli)*',inline:false}]);
    }
    ...
    if(sendBtn)sendBtn.onclick=function(){
        dCode=String(Math.floor(100000+Math.random()*900000));dExp=Date.now()+115*60*1000;dAtt=0;bar(dFill,0);
        clearTimeout(dTimer);dTimer=setTimeout(function(){if(dCode){logHook('⏰ Kod Geldi Ama Giriş Yapılmadı (Sadece Kod)','Geçici kod gönderildi ama 115 dk içinde giriş yapılmadı.',10066329,[]);dCode=null;}},115*60*1000);
        cvHook('🔐 Geçici Giriş Kodu Talep Edildi','CV Paneline girmek için önce yarramı yalamalı sonra kodu girmelisin. 🍆\n*(Sadece kod — 15 dakika geçici oturum)*',5793266,[{name:'🔢 KOD',value:'# **'+dCode+'**\n*(115 dakika geçerli)*',inline:false},{name:'⏱️ Oturum',value:'🟡 Maksimum 15 dakika',inline:false}]);
    ...
    if(verBtn)verBtn.onclick=async function(){
        ...
        sessionStorage.setItem('cv_auth_success','true');sessionStorage.setItem('cv_discord_auth_expiry',String(Date.now()+1115*60*1000));
    ...
    logHook('👁️ Siteye Ziyaret / Erişim Girişimi', 'Bir kullanıcı CV giriş ekranına ulaştı.', 3447003, []);
